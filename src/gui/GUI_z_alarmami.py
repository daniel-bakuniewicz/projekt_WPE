...
class MarineVisionFinal:
    ...
    def ai_loop(self):
        ...
        while self.is_running:
                if self.ai_active and self.current_frame is not None:
                        frame_to_process = self.current_frame.copy()
		
                        #rgb = cv2.cvtColor(frame_to_process, cv2.COLOR_BGR2RGB)
                        interp_input = cv2.resize(frame_to_process, (self.w, self.h))
    ...
    def take_photo(self):
        ...
        if ret:
            ...
            cv2.imwrite(f"foto_{ts}.jpg", bgr_frame)
    ...
    def update_gui(self):
        ...
        if self.camera_ready:
            ...
            for det in self.last_detections:
                ...
                # NAPRAWA: Zabezpieczenie, żeby tekst nie uciekał poza górną krawędź
                text_y = t - 10 if t > 20 else t + 20

                cv2.rectangle(frame, (l, t), (r, b), (0, 0, 255), 2)
                cv2.putText(frame, det['class'], (l, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            ...
            # NAPRAWA: Aktualizacja tego samego obiektu zamiast tworzenia setek tysięcy nowych
            if not hasattr(self, 'canvas_image'):
                self.canvas_image = self.cam_canvas.create_image(0, 0, image=self.tk_img, anchor=tk.NW)
            else:
                self.cam_canvas.itemconfig(self.canvas_image, image=self.tk_img)
